...
The `Android CI` workflow runs on each main-branch push and pull request and uploads a debug-only APK artifact. `Signed beta release` is manual; it requires a `v`-prefixed tag on the selected commit and the four signing secrets. The workflow builds and signs the release APK/AAB, verifies the APK, writes checksums and creates a GitHub prerelease. Do not publish a debug APK as the store build. Keep the release tag immutable once distributed.
